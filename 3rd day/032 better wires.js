    ...
    arr.forEach(arrEl => {
        ...
        switch(arrEl.slice(0, 1)){
            ...
            case 'D':
                ...
                break;
            case 'R':
                for (i=0;i<num;i++) marked.push(`${pos[0]} ${pos[1]+(i+1)}`);
                pos[1] += num;
                break;
            case 'L':
                for (i=0;i<num;i++) marked.push(`${pos[0]} ${pos[1]-(i+1)}`);
...
let oneMark = markPath(oneArr);
let twoMark = markPath(twoArr);
let crossings = oneMark.filter(x => twoMark.includes(x));

function doCounts(arr){
    let countArr = [];
    for (let i=0;i<crossings.length;i++){
        countArr.push(arr.indexOf(crossings[i])+1);
    }
    return countArr;
}
let oneCounts = doCounts(oneMark);
let twoCounts = doCounts(twoMark);

let stepcount = [];
for (i=0;i<oneCounts.length;i++){
    stepcount.push(oneCounts[i]+twoCounts[i]);
}
console.log(Math.min(...stepcount));